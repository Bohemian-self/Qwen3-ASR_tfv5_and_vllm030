...
def _safe_register_auto():
    # transformers 5.x 重复 register 会抛错；且部分版本的 register 签名多了 exist_ok
    for _fn, _args in (
...
def _filter_llm_kwargs(kwargs: Dict[str, Any]):
    # vLLM 0.30 对未知 LLM() 参数直接 TypeError；按签名过滤后再透传
    try:
        import inspect

        from vllm import LLM as _LLMCls

        _sig = inspect.signature(_LLMCls.__init__)
        _params = _sig.parameters
        _has_var_kw = any(p.kind == inspect.Parameter.VAR_KEYWORD for p in _params.values())
        if _has_var_kw:
            return dict(kwargs)
        return {k: v for k, v in kwargs.items() if k in _params}
    except Exception:
        return dict(kwargs)


def _make_sampling_params(max_new_tokens: Optional[int]):
    from vllm import SamplingParams

    _n = int(max_new_tokens) if max_new_tokens is not None else 4096
    # 0.30 SamplingParams 用 max_tokens；旧版混用 max_new_tokens。做双签名兼容
    for _kw in ({"temperature": 0.0, "max_tokens": _n}, {"temperature": 0.0, "max_new_tokens": _n}, {"temperature": 0.0}):
        try:
            return SamplingParams(**_kw)
        except TypeError:
            continue
...
def _vllm_generate(llm, prompts, sampling_params):
    # 0.30 generate() 删除 use_tqdm；按签名决定是否传入
    try:
        import inspect

        _sig = inspect.signature(llm.generate)
        if "use_tqdm" in _sig.parameters:
...
class Qwen3ASRModel:
    ...
    def LLM(
        ...
        **kwargs,
        ...
        # (1)-(5) 保持原优化分支的默认调参；但 0.30 会对未知参数 TypeError，
        # 因此先 setdefault 再按签名过滤，未知参数会被丢弃而不是崩。
        ...
        kwargs = _filter_llm_kwargs(kwargs)
        llm = vLLM(model=model, **kwargs)

        try:
            processor = Qwen3ASRProcessor.from_pretrained(model, trust_remote_code=True)
        except TypeError:
            processor = Qwen3ASRProcessor.from_pretrained(model)
        sampling_params = _make_sampling_params(max_new_tokens)
    ...
    def _infer_asr_transformers(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # BatchFeature.to(device) 在 5.x 不再接受 dtype 链式调用；分开处理
            try:
                _dev = self.model.device
            except Exception:
                try:
            ...
            try:
                _dt = self.model.dtype
            except Exception:
                _dt = None
            try:
                inputs = inputs.to(_dev)
            except Exception:
                inputs = inputs.to(str(_dev))
            if _dt is not None:
                try:
                    # 只转浮点特征，不动 input_ids/attention_mask
                    for _k in ("input_features", "input_values"):
                        if _k in inputs and hasattr(inputs[_k], "to"):
                            inputs[_k] = inputs[_k].to(_dt)
                except Exception:
                    pass

            text_ids = self.model.generate(**inputs, max_new_tokens=self.max_new_tokens)

            # transformers 5.x generate 默认返回 tensor；旧代码按 .sequences 取。做兼容
            try:
                _seq = text_ids.sequences
    ...
    def _infer_asr_vllm(
        ...
        # 0.30 的 multi_modal_data.audio 接受 ndarray；旧代码包 [w] 会被当成多段音频。
        # 这里统一传原始 ndarray，保证 batch/streaming/serve 三路一致。
        ...
        for c, w, fl in zip(contexts, wavs, languages):
            ...
            try:
                ...
                prompt_token_ids = self.processor.tokenizer.encode(prompt)
            _wav = np.asarray(w, dtype=np.float32)
            inputs.append({
                "prompt_token_ids": prompt_token_ids,
                "multi_modal_data": {"audio": _wav},
            })

        outs: List[str] = []
        for batch in chunk_list(inputs, self.max_inference_batch_size):
            outputs = _vllm_generate(self.model, batch, self.sampling_params)
            for o in outputs:
    ...
    def streaming_transcribe(self, pcm16k: np.ndarray, state: ASRStreamingState) -> ASRStreamingState:
        ...
        while state.buffer.shape[0] >= state.chunk_size_samples:
            ...
            # 与批量路径一致:预编码 prompt_token_ids，避免 0.30 对 prompt+audio 二次分词的不一致；
            # 音频统一传 ndarray(非 [ndarray])。
            try:
                _ids = self.processor.tokenizer.encode(prompt, add_special_tokens=False)
            except TypeError:
                _ids = self.processor.tokenizer.encode(prompt)
            inp = {"prompt_token_ids": _ids, "multi_modal_data": {"audio": np.asarray(state.audio_accum, dtype=np.float32)}}

            outputs = _vllm_generate(self.model, [inp], self.sampling_params)
