...
try:
    ...
    from transformers.feature_extraction_utils import BatchFeature  # type: ignore[no-redef]
try:
    from transformers.models.whisper import WhisperFeatureExtractor
except ImportError:
    from transformers import WhisperFeatureExtractor  # type: ignore[no-redef]

# ---- vLLM 0.17 ~ 0.30 兼容导入 ----
# 0.30 对 config/distributed/inputs/attention/registry 做了多轮搬家；
# 这里全部用 try/except 桥接，保证同一份代码在 0.17 和 0.30 都能 import。
try:
    from vllm.config import MultiModalConfig, ModelConfig, SpeechToTextConfig, VllmConfig
except ImportError:
    from vllm.config import VllmConfig, ModelConfig  # type: ignore[no-redef]
    try:
        from vllm.config import MultiModalConfig  # type: ignore[no-redef]
    except ImportError:
        MultiModalConfig = Any  # type: ignore[assignment,misc]
    try:
        from vllm.config import SpeechToTextConfig  # type: ignore[no-redef]
    except ImportError:
        try:
            from vllm.config.stt import SpeechToTextConfig  # type: ignore[no-redef]
        except ImportError:
            SpeechToTextConfig = Any  # type: ignore[assignment,misc]
try:
    from vllm.config.multimodal import BaseDummyOptions
except ImportError:
    try:
        from vllm.config import BaseDummyOptions  # type: ignore[no-redef]
    except ImportError:
        BaseDummyOptions = Any  # type: ignore[assignment,misc]
try:
    from vllm.distributed import get_tensor_model_parallel_world_size
except ImportError:
    from vllm.distributed.parallel_state import (  # type: ignore[no-redef]
...
try:
    ...
    try:
        from vllm.inputs.data import PromptType  # type: ignore[no-redef]
    except ImportError:
        PromptType = Any  # type: ignore[assignment,misc]
from vllm.logger import init_logger
try:
    from vllm.model_executor.layers.activation import _ACTIVATION_REGISTRY
except ImportError:
    try:
        from vllm.model_executor.layers.activation import (  # type: ignore[no-redef]
            ACTIVATION_REGISTRY as _ACTIVATION_REGISTRY,
        )
    except ImportError:
        _ACTIVATION_REGISTRY = {}  # type: ignore[assignment]
try:
    ...
    try:
        from vllm.model_executor.layers.attention.encoder_attention import (  # type: ignore[no-redef]
            MMEncoderAttention,
        )
    except ImportError:
        from vllm.model_executor.layers.attention import MMEncoderAttention  # type: ignore[no-redef]
...
try:
    ...
    from vllm.model_executor.model_loader.loader import default_weight_loader  # type: ignore[no-redef]
try:
    ...
    # 0.30 接口拆分/改名时回退为最小 mixin，保证类定义不崩；功能缺失会在运行时显式报错
    try:
        from vllm.model_executor.models.interfaces import (  # type: ignore[no-redef]
            MultiModalEmbeddings,
            SupportsMultiModal,
            SupportsPP,
        )
    except ImportError:
        MultiModalEmbeddings = Any  # type: ignore[assignment,misc]

        class SupportsMultiModal:  # type: ignore[no-redef]
            pass

        class SupportsPP:  # type: ignore[no-redef]
            pass

    try:
        from vllm.model_executor.models.interfaces import SupportsMRoPE  # type: ignore[no-redef]
    except ImportError:

        class SupportsMRoPE:  # type: ignore[no-redef]
            pass
    ...
    try:
        ...
        class SupportsTranscription:  # type: ignore[no-redef]
            pass

try:
    from vllm.model_executor.models.module_mapping import MultiModelKeys
except ImportError:
    from vllm.model_executor.models.utils import MultiModelKeys  # type: ignore[no-redef]
try:
    from vllm.model_executor.models.qwen3 import Qwen3ForCausalLM
except ImportError:
    from vllm.model_executor.models.qwen3_text import Qwen3ForCausalLM  # type: ignore[no-redef]
try:
    ...
    try:
        # 0.30 可能改名为 qwen3_omni_thinker / qwen2_5_omni_thinker
        from vllm.model_executor.models.qwen3_omni_thinker import (  # type: ignore[no-redef]
            Qwen2_5OmniAudioFeatureInputs,
            Qwen3OmniMoeThinkerMultiModalProcessor,
        )
    except ImportError:
        # 最后回退:用通用 BaseMultiModalProcessor，保证 import 不崩；
        # 音频字段类型退化为 dict，_parse_and_validate_audio_input 仍可工作。
        try:
            from vllm.multimodal.processing import BaseMultiModalProcessor  # type: ignore[no-redef]
        except ImportError:
            BaseMultiModalProcessor = object  # type: ignore[assignment,misc]
        Qwen2_5OmniAudioFeatureInputs = dict  # type: ignore[assignment,misc]
        Qwen3OmniMoeThinkerMultiModalProcessor = BaseMultiModalProcessor  # type: ignore[assignment,misc]
try:
    ...
    from vllm.model_executor.models.utils import (  # type: ignore[no-redef]
        AutoWeightsLoader,
        WeightsMapper,
        maybe_prefix,
    )
    try:
        from vllm.model_executor.models.utils import (  # type: ignore[no-redef]
            merge_multimodal_embeddings as _merge_multimodal_embeddings,
        )
    except ImportError:
        _merge_multimodal_embeddings = None  # type: ignore[assignment]
try:
    from vllm.model_executor.models.whisper import ISO639_1_SUPPORTED_LANGS
except ImportError:
    ISO639_1_SUPPORTED_LANGS = {}  # type: ignore[assignment]
...
try:
    ...
    from vllm.multimodal.processing.context import (  # type: ignore[no-redef]
        BaseProcessingInfo,
    )
    from vllm.multimodal.processing.prompt_update import (  # type: ignore[no-redef]
        PromptReplacement,
        PromptUpdate,
    )
try:
    ...
    try:
        from vllm.forward_context import IntermediateTensors  # type: ignore[no-redef]
    except ImportError:
        IntermediateTensors = Any  # type: ignore[assignment,misc]
try:
    from vllm.v1.attention.backends.registry import AttentionBackendEnum
except ImportError:
    try:
        from vllm.attention.backends.registry import AttentionBackendEnum  # type: ignore[no-redef]
    except ImportError:
        try:
            from vllm.attention import AttentionBackendEnum  # type: ignore[no-redef]
        except ImportError:
            AttentionBackendEnum = Any  # type: ignore[assignment,misc]
try:
    from vllm.tokenizers import cached_tokenizer_from_config
except ImportError:
    from vllm.transformers_utils.tokenizer import (  # type: ignore[no-redef]
...
try:
    ...
    try:
        from vllm.transformers_utils.processors import (  # type: ignore[no-redef]
            cached_processor_from_config,
        )
    except ImportError:
        from vllm.multimodal.processing import (  # type: ignore[no-redef]
            cached_processor_from_config,
        )
try:
    ...
    try:
        from vllm.attention import get_vit_attn_backend  # type: ignore[no-redef]
    except ImportError:

        def get_vit_attn_backend(*args, **kwargs):  # type: ignore[misc]
            return None
...
try:
    ...
    from vllm.multimodal.processing.builder import (  # type: ignore[no-redef]
...
def _resolve_activation_fn(name: str):
    # _ACTIVATION_REGISTRY 在 0.30 可能改名/变对象；做兼容
    try:
        if isinstance(_ACTIVATION_REGISTRY, dict):
            return _ACTIVATION_REGISTRY[name]
        getter = getattr(_ACTIVATION_REGISTRY, "get", None)
        if callable(getter):
            fn = getter(name)
            if fn is not None:
                return fn
        return _ACTIVATION_REGISTRY[name]  # type: ignore[index]
    except Exception:
        # 回退常用激活，避免整模型不可用
        _fallback = {"gelu": torch.nn.GELU(), "gelu_new": torch.nn.GELU(), "relu": torch.nn.ReLU(), "silu": torch.nn.SiLU()}
...
class Qwen3ASRAudioEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.activation_fn = _resolve_activation_fn(config.activation_function)
...
class Qwen3ASRAudioEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.act = _resolve_activation_fn(config.activation_function)
    ...
    def compute_attn_mask_seqlen(self, cu_seqlens: torch.Tensor) -> torch.Tensor | None:
        ...
        max_seqlen = None
        try:
            _flash = getattr(AttentionBackendEnum, "FLASH_ATTN", None)
            _rocm = getattr(AttentionBackendEnum, "ROCM_AITER_FA", None)
            _backends = {b for b in (_flash, _rocm) if b is not None}
            if _backends and self.attn_backend in _backends:
                max_seqlen = (cu_seqlens[1:] - cu_seqlens[:-1]).max()
            elif not _backends:
                # AttentionBackendEnum 不可用时保守返回 max，功能不受影响
                max_seqlen = (cu_seqlens[1:] - cu_seqlens[:-1]).max()
        except Exception:
            max_seqlen = None
        return max_seqlen
...
class Qwen3ASRProcessingInfo(BaseProcessingInfo):
    ...
    # vLLM 0.16+ 把 BaseProcessingInfo.get_data_parser 改名为 build_data_parser；
    # 0.30 只调新名字。保留双入口，避免 ValueError: BaseMultiModalProcessor._get_data_parser has been moved
...
class Qwen3ASRForConditionalGeneration(
    ...
    def embed_input_ids(
        ...
        *,
        ...
        try:
            ...
            # 0.30 的 _embed_text_input_ids 签名变化时回退
            inputs_embeds = self.language_model.embed_input_ids(input_ids)

        if multimodal_embeddings is None or len(multimodal_embeddings) == 0:
            return inputs_embeds

        if _merge_multimodal_embeddings is None:
            # 0.30 删除私有 _merge_... 时，用 vLLM 通用 merge 逻辑回退
            try:
                from vllm.model_executor.models.utils import (  # type: ignore
                    merge_multimodal_embeddings,
                )

                return merge_multimodal_embeddings(inputs_embeds, multimodal_embeddings, is_multimodal)
            except Exception:
                return inputs_embeds
    ...
    def get_speech_to_text_config(
        ...
        # 0.30 的 SpeechToTextConfig 字段可能改名/新增必填，做双签名兼容
        _chunk = getattr(feature_extractor, "chunk_length", 30)
        _sr = getattr(feature_extractor, "sampling_rate", 16000)
        try:
            return SpeechToTextConfig(
                max_audio_clip_s=_chunk,
                sample_rate=_sr,
            )
        except TypeError:
            try:
    ...
    def get_generation_prompt(
        ...
        # ISO639_1_SUPPORTED_LANGS 在 0.30 可能从 dict 变成 list/set；做兼容
        try:
            _langs = cls.supported_languages
            if hasattr(_langs, "get"):
                full_lang_name_to = _langs.get(effective_language, effective_language)
            elif isinstance(_langs, (list, tuple, set)):
                full_lang_name_to = effective_language
            else:
                full_lang_name_to = effective_language
        except Exception:
            full_lang_name_to = effective_language
