...
except ImportError:  # transformers>=5.18 精简 integrations 时回退为 no-op
...
except ImportError:  # 极旧版本无 masking_utils
...
try:
    ...
    try:
        from transformers.modeling_rope_utils import dynamic_rope_update  # type: ignore

        ROPE_INIT_FUNCTIONS = {}  # type: ignore
    except ImportError:
...
try:
    ...
    try:
        ...
        from typing import Unpack  # type: ignore[type-arg]
from transformers.utils import auto_docstring, can_return_tuple
from transformers.utils.deprecation import deprecate_kwarg
try:
    from transformers.utils.generic import TransformersKwargs, check_model_inputs
except ImportError:
    # transformers 5.x 把 TransformersKwargs 提升到 transformers.utils
    from transformers.utils import TransformersKwargs  # type: ignore
    try:
        from transformers.utils.generic import check_model_inputs  # type: ignore
    except ImportError:
        try:
            from transformers.utils import check_model_inputs  # type: ignore
        except ImportError:

            def check_model_inputs(*dargs, **dkwargs):  # type: ignore[misc]
                # 兼容 v5: check_model_inputs 不再是工厂函数。
                # 支持 @check_model_inputs 和 @check_model_inputs() 两种写法。
...
def _resolve_attention_interface(attn_impl: str):
    """兼容 transformers 4.x(dict) / 5.x(AttentionInterface.get_interface)."""
    try:
        getter = getattr(ALL_ATTENTION_FUNCTIONS, "get_interface", None)
        if callable(getter):
            try:
                return getter(attn_impl, eager_fallback=True)
            except TypeError:
                return getter(attn_impl)
    except Exception:
        pass
    # 回退:旧版 dict 行为;未知实现则回退 eager
    try:
        return ALL_ATTENTION_FUNCTIONS[attn_impl]  # type: ignore[index]
    except Exception:
        try:
            return ALL_ATTENTION_FUNCTIONS["eager"]  # type: ignore[index]
        except Exception:
            return eager_attention_forward


def _get_rope_params(config):
    """兼容 rope_theta/rope_scaling(4.x) 与 rope_parameters(5.x)."""
    rope_type = "default"
    rope_theta = getattr(config, "rope_theta", 5000000.0)
    rope_scaling = getattr(config, "rope_scaling", None)
    rope_params = getattr(config, "rope_parameters", None)
    if isinstance(rope_params, dict) and rope_params:
        rope_type = rope_params.get("rope_type", rope_type)
        rope_theta = rope_params.get("rope_theta", rope_theta)
        rope_scaling = rope_scaling if rope_scaling is not None else rope_params
    elif isinstance(rope_scaling, dict):
        rope_type = rope_scaling.get("rope_type", rope_scaling.get("type", "default"))
    return rope_type, rope_theta, rope_scaling


def _make_causal_mask(*, config, inputs_embeds, attention_mask, past_key_values, position_ids, cache_position=None):
    """兼容 create_causal_mask 在 4.x/5.x 的签名差异(input_embeds/cache_position 改名/删除)."""
    if create_causal_mask is None:
        return attention_mask
    # 5.x: inputs_embeds(复数),无 cache_position; 4.x: input_embeds(单数)+cache_position
    for kwargs in (
        ...
        {"config": config, "input_embeds": inputs_embeds, "attention_mask": attention_mask,
        ...
        try:
            return create_causal_mask(**kwargs)  # type: ignore[operator]
        except TypeError:
            continue
    return attention_mask
...
class Qwen3ASRTextAttention(nn.Module):
    ...
    # transformers 5.x 已删除单数 past_key_value，且 deprecate_kwarg 的 version 过期会告警；做兼容包装
    def forward(
        self,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # 手动兼容旧单数参数名，避免依赖已过期的 deprecate_kwarg(version="4.58")
        if "past_key_value" in kwargs and past_key_values is None:
        ...
        if past_key_values is not None:
            ...
            # transformers 5.x 的 Cache.update 已不再接受 sin/cos，做双签名兼容
            try:
...
class Qwen3ASRPreTrainedModel(PreTrainedModel):
    ...
    # transformers 5.x 要求 list；str 会被逐字符迭代导致 device placement 异常
...
class Qwen3ASRThinkerTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen3ASRConfig, device=None):
        super().__init__()
        rope_type, rope_theta, rope_scaling = _get_rope_params(config)
        self.rope_type = rope_type
        self.max_seq_len_cached = getattr(config, "max_position_embeddings", 128000)
        self.original_max_seq_len = getattr(config, "max_position_embeddings", 128000)

        self.config = config
        # transformers 5.x 的 ROPE_INIT_FUNCTIONS 没有 "default" 键，回退 linear/default 初始化
        rope_fn = None
        try:
            rope_fn = ROPE_INIT_FUNCTIONS[self.rope_type]  # type: ignore[index]
        except Exception:
            for _k in ("default", "linear", "dynamic"):
                try:
                    rope_fn = ROPE_INIT_FUNCTIONS[_k]  # type: ignore[index]
                    break
                except Exception:
                    continue
        if rope_fn is None:
            # 最后回退:标准 llama 式 inv_freq，避免直接 KeyError 导致整模型不可用
            def _fallback_rope_init(cfg, dev):
                dim = getattr(getattr(cfg, "text_config", cfg), "head_dim", None) or (
                    getattr(cfg, "hidden_size", 4096) // getattr(cfg, "num_attention_heads", 32)
                )
                _theta = getattr(cfg, "rope_theta", None) or rope_theta
                _inv = 1.0 / (_theta ** (torch.arange(0, dim, 2).float() / dim))
                return _inv, 1.0

            rope_fn = _fallback_rope_init
        self.rope_init_fn = rope_fn
        ...
        try:
            _rs = getattr(config, "rope_scaling", None) or getattr(config, "rope_parameters", None) or {}
            if isinstance(_rs, dict):
                self.mrope_section = _rs.get("mrope_section", self.mrope_section)
        except Exception:
            pass
...
class Qwen3ASRThinkerTextModel(Qwen3ASRPreTrainedModel):
    ...
    # transformers 5.x: check_model_inputs 不再是工厂函数，@check_model_inputs() 会失败；用 try 兼容两种写法
    try:
        _check_decorator = check_model_inputs()
    except Exception:
        _check_decorator = check_model_inputs
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if use_cache and past_key_values is None and not torch.jit.is_tracing():
            # transformers 5.x 的 DynamicCache 需要 text_config(含 num_hidden_layers/layer_types)；
            # 顶层 Qwen3ASRConfig 是嵌套结构，直接传入会取不到字段，做回退。
            try:
                past_key_values = DynamicCache(config=self.config)
            except Exception:
                try:
                    _tc = self.config.get_text_config() if hasattr(self.config, "get_text_config") else self.config
                    past_key_values = DynamicCache(config=_tc)
                except Exception:
        ...
        if cache_position is None:
            try:
                ...
                # transformers 5.x CacheLayer 结构下旧 get_seq_length 语义变化，失败则按 0 处理
                # (首轮 prefill 本就该从 0 开始；decode 轮 cache_position 由 generate 传入，不走这里)
                past_seen_tokens = 0
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            # transformers 5.x: loss_function(logits, labels, config)；4.x: (logits, labels, vocab_size)
            try:
                ...
                try:
                    loss = self.loss_function(
                        logits=logits, labels=labels, config=self.config.get_text_config()
                    )
                except TypeError:
...
class Qwen3ASRForConditionalGeneration(Qwen3ASRPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # transformers 5.x generate 新增 generation_config/cache_implementation，透传以免 TypeError/静默丢参
        if generation_config is not None:
            thinker_kwargs["generation_config"] = generation_config
        if cache_implementation is not None:
            thinker_kwargs["cache_implementation"] = cache_implementation
        ...
        # 兼容 5.x generate 对 return_dict_in_generate 的默认值变化，显式保留旧行为
        thinker_kwargs.setdefault("return_dict_in_generate", True)
