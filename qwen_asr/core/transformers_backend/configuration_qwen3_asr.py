...
class Qwen3ASRTextConfig(PretrainedConfig):
    ...
    def rope_parameters(self):
        # transformers 5.x 统一经 config.rope_parameters 访问 RoPE；
        # 本项目旧 checkpoint 只有 rope_theta/rope_scaling，在此做只读桥接。
        _rs = getattr(self, "rope_scaling", None) or {}
        _theta = getattr(self, "rope_theta", 5000000.0)
        if isinstance(_rs, dict) and _rs:
            out = dict(_rs)
            out.setdefault("rope_type", out.get("type", "default"))
            out.setdefault("rope_theta", _theta)
            return out
        return {"rope_type": "default", "rope_theta": _theta}
...
class Qwen3ASRThinkerConfig(PretrainedConfig):
    ...
    def get_text_config(self, decoder=False) -> "PretrainedConfig":
        # transformers 5.x 的 DynamicCache(config=...) 会调用 config.get_text_config(decoder=True)
        # 取 num_hidden_layers/layer_types；这里 asr->thinker->text 深嵌套，必须显式转发到底层 text_config
        try:
            _tc = getattr(self, "text_config", None)
            if _tc is not None:
                return _tc
        except Exception:
            pass
        try:
            return super().get_text_config(decoder=decoder)  # type: ignore[misc]
        except Exception:
            return getattr(self, "text_config")
...
class Qwen3ASRConfig(PretrainedConfig):
    ...
    def get_text_config(self, decoder=False) -> "PretrainedConfig":
        ...
        try:
            inner = self.thinker_config.get_text_config(decoder=decoder)
            # thinker_config.get_text_config 可能回退到 thinker 本身(无 vocab_size)；
            # 若如此则继续下钻到 text_config，保证 DynamicCache/vLLM 拿到真正的文本配置
            if getattr(inner, "vocab_size", None) is None:
                _tc = getattr(getattr(self, "thinker_config", None), "text_config", None)
                if _tc is not None:
                    return _tc
            return inner
        except Exception:
