...
try:
    ...
    from transformers.tokenization_utils_base import AudioInput  # type: ignore[no-redef]
try:
...
class Qwen3ASRProcessor(ProcessorMixin):
    ...
    # transformers 5.x 要求子类声明 valid_processor_kwargs；缺失会导致 _merge_kwargs 走默认分支
    valid_processor_kwargs = Qwen3ASRProcessorKwargs

    def __init__(
        self, feature_extractor=None, tokenizer=None, chat_template=None
    ):
        # transformers 5.x 的 ProcessorMixin.__init__ 严格校验 kwargs 数量；
        # 显式以关键字传入，避免位置参数在 attributes 顺序变化时错位。
        super().__init__(
            feature_extractor=feature_extractor, tokenizer=tokenizer, chat_template=chat_template
        )
        # 部分 tokenizer(如社区/旧 checkpoint)在 5.x 下缺 audio_token 属性，做 getattr 回退
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if audio is not None:
            ...
            # transformers 4.x 返回 attention_mask；5.x 可能直接返回 feature_attention_mask。两种都兼容。
            if "feature_attention_mask" not in audio_inputs and "attention_mask" in audio_inputs:
                audio_inputs["feature_attention_mask"] = audio_inputs.pop(
                    "attention_mask"
                )  # rename to prevent conflicts with text attention_mask
            # input_features 在新版可能叫 input_values，做兼容(保留原 key，不做无意义 pop)
            if "input_features" not in audio_inputs and "input_values" in audio_inputs:
        ...
        # transformers 5.x 要求 return_tensors 从 merged text_kwargs 取；kwargs.get 可能取不到
        _rt = kwargs.get("return_tensors", None)
        if _rt is None:
            try:
                _rt = output_kwargs.get("text_kwargs", {}).get("return_tensors", None)
            except Exception:
                _rt = None
        return BatchFeature(
